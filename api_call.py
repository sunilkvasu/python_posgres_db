...
def update_post():
    url = "http://sacvl395.sac.flextronics.com:4210/" + action + "/" + token + "/" + patchid + "/" + status + "/" + date
    try:
        ...
        return None
def insert_post():
    url = "http://sacvl395.sac.flextronics.com:4210/" + action + "/" + token + "/" + patchid + "/" + serverid + "/" + changeid + "/" + own + "/" + app + "/" + status + "/" + date
    try:
        response = requests.get(url)
        print('Response code:', response.status_code)
        print(response.content)
        ...
        return None
def delete_post():
    url = "http://sacvl395.sac.flextronics.com:4210/" + action + "/" + token + "/" + patchid + "/" + serverid
    try:
        response = requests.get(url)
        print('Response code:', response.status_code)
        print(response.content)
